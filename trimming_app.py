...
class Model():
    ...
    def get_image(self, type):
        ...
        if type == Model.BEFORE:
            if self.before_image is not None:
                # Tkinter画像オブジェクトに変換
                self.before_image_tk = ImageTk.PhotoImage(
                    self.before_image, width = 3280, height = 2464)
            return self.before_image_tk
        elif type == Model.AFTER:
            if self.after_image is not None:
                # Tkinter画像オブジェクトに変換
                self.after_image_tk = ImageTk.PhotoImage(
                    self.after_image)
    ...
    def read(self, path):
        ...
        self.before_image_original = Image.open(path)
        (width, height) = (self.before_image_original.width//8 , self.before_image_original.height//8)
        # 画像をリサイズする
        self.before_image = self.before_image_original.resize((width, height))
...
class Controller():
    ...
    def __init__(self, app, model, view):
        ...
        self.selection = None
        # ラベル表示メッセージ管理用
        self.message = "トリミングする部分を選んでください"
        self.set_events()
        if len(file_path) != 0:
    ...
    def button_release(self, event):
        ...
        if self.pressing:
            ...
            if len(objs) != 0:
                ...
                self.image_after = self.model.crop(
                    (int(x1), int(y1), int(x2), int(y2))
                )
           
                self.image_after=img.crop(
                    (int(x1*8),int(y1*8),int(x2*8),int(y2*8))
                )
                self.image_after.save('final.jpg')
                # メッセージを更新
...
def trimming(file_patha):
        ...
        app.title("トリミングアプリ")
        global img ; img = Image.open(file_path)
        model = Model()
        view = View(app, model)
